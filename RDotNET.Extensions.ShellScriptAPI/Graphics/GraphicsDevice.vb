...
Once you have done the function from this namespace, then you can using the Shoal Shell hybrid scripting feature to draw the image and save to a file.
    ...
    Public Module GraphicsDevice
        ...
        Public Function bmp(plot As String,
                        ...
                        <Parameter("pointsize", "the default pointsize of plotted text, interpreted as big points (1/72 inch) at res ppi.")> Optional pointsize As Integer = 12,
                        <Parameter("bg", "the initial background colour: can be overridden by setting par(""bg"").")> Optional bg As String = "white",
                        <Parameter("res", "The nominal resolution in ppi which will be recorded in the bitmap file, if a positive integer. Also used for units other than the default. 
                        If not specified, taken as 72 ppi to set the size of text and line widths.")> Optional res As String = NA,
                        <Parameter("family", "A length-one character vector specifying the default font family. 
        ...
        Public Function jpeg(plot As String,
                        ...
                        (The result must be less than 600 characters long. See postscript for further details.) ")> filename As String,
                         ...
                         <Parameter("units", "The units in which height and width are given. Can be px (pixels, the default), in (inches), cm or mm.")> Optional units As String = "px",
                         <Parameter("pointsize", "the default pointsize of plotted text, interpreted as big points (1/72 inch) at res ppi.")> Optional pointsize As Integer = 12,
                         <Parameter("quality", "the ‘quality’ of the JPEG image, as a percentage. Smaller values will give more compression but also more degradation of the image.")> Optional quality As Integer = 75,
                          <Parameter("bg", "the initial background colour: can be overridden by setting par(""bg"").")> Optional bg As String = "white",
                         <Parameter("res", "The nominal resolution in ppi which will be recorded in the bitmap file, if a positive integer. Also used for units other than the default. 
                        If not specified, taken as 72 ppi to set the size of text and line widths.")> Optional res As String = NA,
                         <Parameter("family", "A length-one character vector specifying the default font family. 
    ...
    dev.off()"
        ...
        Public Function tiff(plot As String,
                        ...
                        <Parameter("units", "The units in which height and width are given. Can be px (pixels, the default), in (inches), cm or mm.")> Optional units As String = "px",
                        <Parameter("pointsize", "the default pointsize of plotted text, interpreted as big points (1/72 inch) at res ppi.")> Optional pointsize As Integer = 12,
                       <Parameter("compression", "the type of compression to be used.")> Optional compression As String = "c(""none"", ""rle"", ""lzw"", ""jpeg"", ""zip"", ""lzw+p"", ""zip+p"")",
                          <Parameter("bg", "the initial background colour: can be overridden by setting par(""bg"").")> Optional bg As String = "white",
                          <Parameter("res", "The nominal resolution in ppi which will be recorded in the bitmap file, if a positive integer. Also used for units other than the default. 
                        If not specified, taken as 72 ppi to set the size of text and line widths.")> Optional res As String = NA,
                          <Parameter("family", "A length-one character vector specifying the default font family. 
