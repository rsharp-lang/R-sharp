#Region "Microsoft.VisualBasic::d6474a5977f454c6dec04573b2526d36, R#\test\debuggerTest.vb"
    ...
    '   Total Lines: 191
    '    Code Lines: 116 (60.73%)
    ' Comment Lines: 35 (18.32%)
    '    - Xml Docs: 28.57%
    ' 
    '   Blank Lines: 40 (20.94%)
    '     File Size: 8.61 KB
    ...
    ' 
    '     Sub: Assert, DumpBreakpoints, Main, RunConditionalBreakpointTest, RunScriptBreakpointTest
    '          RunStepTest
    ' 
    ' /********************************************************************************/
...
"print(d);"
    ...
    Private Sub RunScriptBreakpointTest()
        ...
        '在第三行(function定义)与第八行(c <- f(a))处设置断点
        ...
        Call Assert(hits(0) = 3, "first breakpoint should be at line3")
        Call Assert(hits(1) = 8, "second breakpoint should be at line8")
    ...
    Private Sub RunStepTest()
        ...
        AddHandler dbg.OnBreakpointHit, Sub(frame As DebugFrame)
                                            ...
                                            ElseIf stepCount = 2 Then
                                                '第二次(步过之后):此时应该停在 b <-2
                                                Call Assert(frame.line = 2, $"step over should land on line2, but got {frame.line}")
                                                '步入函数调用
                                                Call dbg.Resume(DebugAction.StepInto)
                                            ElseIf frame.breakpoint Is Nothing AndAlso stepCount >= 3 Then
                                                '步进行走期间:当进入函数体内部时执行 StepOut返回
    ...
    Private Sub RunConditionalBreakpointTest()
        ...
        '在第八行设条件断点:只有当 a大于0的时候才中断
        '由于在脚本里面 a始终为1,所以这个条件断点会被命中
