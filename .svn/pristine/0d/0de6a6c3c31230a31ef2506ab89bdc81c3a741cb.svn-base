...
Public Module PieChart
    ...
    Public Function PieSimple(slices As Generic.IEnumerable(Of KeyValuePair(Of String, Double)),
...
{GraphicsDevice.tiff(plot:=$"pie(slices, labels = lbls, main=""{Title}"")", filename:=SaveTo, width:=3000, height:=2500)}"
    ...
    Public Function DataFrame(path As String) As KeyValuePair(Of String, Double)()
        ...
        Dim LQuery = (From row In Csv.Skip(1).AsParallel
                      Select New KeyValuePair(Of String, Double)(row(0), Val(row(1)))).ToArray
        Return LQuery
    End Function

    <ExportAPI("Data.Frame")>
    Public Function DataFrame(data As Generic.IEnumerable(Of Object)) As KeyValuePair(Of String, Double)()
        Dim LQuery = (From obj In data.AsParallel
                      Let values = Microsoft.VisualBasic.Scripting.InputHandler.CastArray(Of Object)(obj)
                      Select New KeyValuePair(Of String, Double)(
                          Microsoft.VisualBasic.Scripting.InputHandler.ToString(values(0)),
                          Val(Microsoft.VisualBasic.Scripting.InputHandler.ToString(values(1))))).ToArray
        Return LQuery
    End Function
