#Region "Microsoft.VisualBasic::f57a2a81ddcf5fba1c7cd419088d3f75, Library\Rlapack\RMatrix.vb"
...
'   Total Lines: 880
'    Code Lines: 512 (58.18%)
' Comment Lines: 250 (28.41%)
'    - Xml Docs: 88.40%
' 
'   Blank Lines: 118 (13.41%)
'     File Size: 34.53 KB
...
'               nmf_matrix, one, parse, readMatrix, rowPack
'               saveMatrix, (+3 Overloads) substract, sum_all, zero
' 
'     Sub: extractVector
' 
' /********************************************************************************/
...
Module RMatrix
    ...
    Public Function saveDataMatrix(m As DataMatrix, args As list, env As Environment) As Object
        ...
        Call MatrixFormat.WriteData(m, con, compress:=True)
        Call con.Flush()
        Return True
    End Function
    ...
    ''' <summary>
    ''' supports convert the dataframe/generalmatrix/list of vector/vector of vector to matrix object
    ''' </summary>
    ''' <param name="x"></param>
    ''' <param name="env"></param>
    ''' <returns></returns>
    Private Function matrix_extractor(x As Object, env As Environment) As [Variant](Of NumericMatrix, Message)
        ...
        If x Is Nothing Then
            Return RInternal.debug.stop("missing input data for matrix conversion!", env)
        End If
        ...
        ElseIf x.GetType.ImplementInterface(Of INumericMatrix) Then
            data = New NumericMatrix(DirectCast(x, INumericMatrix))
